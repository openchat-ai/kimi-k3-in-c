...
typedef struct {
    ...
    /* Unified media-tier I/O scheduler. tier[i] selects the scheduler tier for
     * shard i: experts live on the slow checkpoint (HDD tier), the embed/lm_head
     * shard, when redirected to sdd7, is NVMe. NULL = direct pread. */
    K3IO       *kio;
    int        *tier;         /* [nshard] scheduler tier, or -1 for direct */
} K3St;
