...
static void *k3_io_tier_main(void *arg)
{
    ...
    for (;;) {
         ...
         * streams share the drive concurrently -- no 0/1 gate. HDD is one worker:
         * those reads are the slow checkpoint, inherently serial, and isolated
         * from the NVMe pool so they cannot stall trunk. */
        ssize_t rc;
        if (r->chunk > 0 && r->nbytes > r->chunk) {
...
void k3_io_init(K3IO *io, int tiers, const int *workers)
{
    ...
    for (int t = 0; t < tiers; t++) {
        ...
        io->nworkers[t] = workers ? workers[t] : 1;
