...
Every number here is copied verbatim from a real capture under
kimi-k3-c/blog_artifacts_kimi_k3/ (no live rerun needed). Style: white background,
one consistent categorical palette (the same hues as the hand-drawn diagram
strokes), light grid, no chartjunk.
...
Chart type is chosen per dataset rather than defaulting to bars:
  ...
  lollipop    a ranking of scalars           storage_regimes, io_share, ...
...
def binary_sizes():
    """LOLLIPOP. 01_build/linux_verification.txt section 1 binary sizes."""
    names = ["k3_run", "test_ops", "k3_model", "scale_test",
             "test_cache", "test_st", "test_cfg"]
    kb = [v / 1024.0 for v in
          [179736, 97784, 89392, 72168, 61240, 55976, 48120]]
    fig, ax = plt.subplots(figsize=(9.0, 3.8))
    lollipop(ax, names, kb, [GREEN] + [GRAY] * 6, fmt="{:.0f}", unit=" KB")
    style(ax, "A 176 KB binary that runs a 1.56 TB model", "kilobytes", grid="x")
