"""Generate all subsection Mermaid (.mmd) sources for the Kimi K3-in-C blog.

Locked style (via mmdc.json): hand-drawn look on warm paper, soft colored fills +
medium warm borders on leaf boxes, dark warm ink for text. Each diagram is simple
(3-8 nodes) so it never overlaps and renders wide + short.
"""
...
add("kernel-contract", """
flowchart LR
    W["same weights"] --> SC["scalar path"]
    W --> OMP["OpenMP path"]
    W --> AVX["AVX2 path"]
    SC --> H{"FNV hash<br/>of the output"}
    OMP --> H
    AVX --> H
    H -->|"identical"| OK["bit for bit"]
""", {"W": "gray", "SC": "blue", "OMP": "amber", "AVX": "orange", "H": "purple", "OK": "green"})
